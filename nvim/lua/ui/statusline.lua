...
for k, v in pairs(ICON) do
  ICON[k] = _G.tools.hl_str(v[1], v[2])
end
...
local function path_widget(root, fname)
  ...
  path = _G.tools.hl_str(hl, icon) .. file_name
  ...
  local remote = _G.tools.get_git_remote_name(root)
  local branch = _G.tools.get_git_branch(root)
  local repo_info = ""
...
local function diagnostics_widget()
  if not _G.tools.diagnostics_available() then return "" end
  ...
  return string.format(
    "%s %s  %s %s  ",
    ICON.error,
    _G.tools.hl_str("DiagnosticError", err),
    ICON.warn,
    _G.tools.hl_str("DiagnosticWarn", warn)
  )
end
...
local function fileinfo_widget()
  ...
  local lines = _G.tools.group_number(api.nvim_buf_line_count(0), ",")
  local str = ICON.fileinfo .. " "

  if not _G.tools.nonprog_modes[ft] then
  ...
  if not wc.visual_words then
    return str
      .. string.format(
        "%3s lines  %3s words",
        lines,
        _G.tools.group_number(wc.words, ",")
      )
  end
  ...
  return str
    .. string.format(
      ...
      _G.tools.group_number(vlines, ","),
      _G.tools.group_number(wc.visual_words, ","),
      _G.tools.group_number(wc.visual_chars, ",")
    )
end
...
local function venv_widget()
  ...
  if env and env ~= "" then
    ...
    return _G.tools.hl_str("Comment", str)
  end
  ...
  return _G.tools.hl_str("Comment", "[no venv]")
end
...
local function scrollbar_widget()
  ...
  return _G.tools.hl_str("Substitute", SBAR[idx]:rep(2))
end

-- render ---------------------------------------------
function M.render()
  local fname = api.nvim_buf_get_name(0)
  local root = (bo.buftype == "" and _G.tools.get_path_root(fname)) or nil
