...
# Portable copy of omarchy's viewer with fallbacks for non-omarchy systems.
...
if command -v omarchy-menu-select >/dev/null 2>&1; then
  ...
  printf '%s\n' "$records" | fzf --height "$menu_height" --delimiter='→' --prompt="Tmux keybindings> " >/dev/null || true
else
  printf '%s\n' "$records" | less -R
fi