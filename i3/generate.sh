...
emacs_theme_name=${EMACS_THEME:-nezburn}
{
  echo ";; Auto-generated by generate.sh - do not edit"
  if [ -n "$CATPPUCCIN_FLAVOR" ]; then
    echo "(setq catppuccin-flavor '$CATPPUCCIN_FLAVOR)"
  fi
  echo "(dolist (th (copy-sequence custom-enabled-themes)) (unless (eq th '$emacs_theme_name) (disable-theme th)))"
  echo "(load-theme '$emacs_theme_name t)"
  echo "(let* ((face (car (delq nil (mapcar (lambda (s) (when (and (consp s) (eq (car s) (quote theme-face)) (eq (nth 1 s) (quote cursor))) (nth 3 s))) (get (quote $emacs_theme_name) (quote theme-settings))))))"
  echo "       (color (and face (plist-get (face-spec-choose face (frame-live-p (selected-frame))) (quote :background)))))"
  echo "  (when color (dolist (frame (frame-list)) (set-frame-parameter frame (quote cursor-color) color))))"
} > "$emacs_theme_file"
